...
const getGenrePopularity = async (req, res) => {
  const query = `
    ...
    JOIN Track tr ON pt.track_id = tr.id
    JOIN ArtistTags at ON tr.artist_id = at.artist_id
    JOIN Tags t ON at.tag_id = t.id
    JOIN Artist a ON tr.artist_id = a.id
...
const getPopularCollaborations = async (req, res) => {
  const query = `
    WITH CollabPlaylists AS (
      SELECT p.id
      FROM Playlist p
      WHERE p.collaborative = TRUE AND p.num_followers > 1000
    ),
    RankedArtists AS (
      SELECT 
        t.artist_id,
        COUNT(DISTINCT pt.playlist_id) AS appearances,
        DENSE_RANK() OVER(PARTITION BY t.artist_id ORDER BY COUNT(DISTINCT pt.playlist_id) DESC) AS rank
      FROM Track t
      JOIN PlaylistTrack pt ON t.id = pt.track_id
      JOIN CollabPlaylists cp ON pt.playlist_id = cp.id
      GROUP BY t.artist_id
    )
    SELECT a.name, ra.appearances
    FROM RankedArtists ra
    JOIN Artist a ON ra.artist_id = a.id
    WHERE ra.rank = 1 AND ra.appearances > 5
