...
function MapPage() {
  ...
  useEffect(() => {
    if (country) {
      setLoading(true);
      Promise.all([
        fetch(`${BACKENDURL}/artists/stats/${country}`),
        fetch(`${BACKENDURL}/genres/top/${country}`),
      ])
        ...
        .catch((error) => {
          ...
          setStats({ total_playlists: 0, avg_tracks_per_playlist: 0 });
          setGenres([]);
          setLoading(false);
        });
    }
  ...
  function validateStats(data) {
    ...
    if (!data.total_playlists || isNaN(avgTracksPerPlaylist)) {
      isValid = false;
    }
    return {
      valid: isValid,
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "1rem" }}>
        {country ? (
          <div>
            ...
            {loading ? (
              <p>Loading playlist statistics...</p>
            ) : stats.valid ? (
              <div>
                ...
                <h3>
                  ...
                  {stats.avg_tracks_per_playlist.toFixed(2)}
                </h3>
              </div>
            ) : (
            ...
            {loading ? (
              ...
              genres.map((genre) => (
                <GenreCard key={genre.tag_name} genre={genre} />
              ))
            )}
          </div>
        ) : (
