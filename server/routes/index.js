...
function fetchTodos(req, res, next) {
    connection.query('SELECT * FROM todos WHERE owner_id = ?', [req.user.id], function(err, rows) {
...
router.post('/', ensureLoggedIn, function(req, res, next) {
    ...
    connection.query('INSERT INTO todos (owner_id, title, completed) VALUES (?, ?, ?)', [
      req.user.id,
      req.body.title,
...
router.post('/:id(\\d+)', ensureLoggedIn, function(req, res, next) {
    ...
    connection.query('UPDATE todos SET title = ?, completed = ? WHERE id = ? AND owner_id = ?', [
      req.body.title,
...
router.post('/:id(\\d+)/delete', ensureLoggedIn, function(req, res, next) {
    connection.query('DELETE FROM todos WHERE id = ? AND owner_id = ?', [
      req.params.id,
      req.user.id
    ], function(err) {
      if (err) { return next(err); }
...
router.post('/toggle-all', ensureLoggedIn, function(req, res, next) {
    connection.query('UPDATE todos SET completed = ? WHERE owner_id = ?', [
      req.body.completed !== undefined ? 1 : null,
      req.user.id
    ], function(err) {
      if (err) { return next(err); }
...
router.post('/clear-completed', ensureLoggedIn, function(req, res, next) {
    connection.query('DELETE FROM todos WHERE owner_id = ? AND completed = ?', [
      req.user.id,
      1
    ], function(err) {
      if (err) { return next(err); }
